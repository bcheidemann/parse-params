...
export function transformTypeScript(context) {
    ...
    function namespaceElementVisitorWorker(node) {
        if (node.kind === SyntaxKind.ExportDeclaration || node.kind === SyntaxKind.ImportDeclaration || node.kind === SyntaxKind.ImportClause || node.kind === SyntaxKind.ImportEqualsDeclaration && node.moduleReference.kind === SyntaxKind.ExternalModuleReference) {
            return undefined;
    ...
    function visitSourceFile(node) {
        ...
        return factory.updateSourceFile(node, visitLexicalEnvironment(node.statements, sourceElementVisitor, context, 0, alwaysStrict));
    }
    ...
    function getClassFacts(node) {
        ...
        if (some(getProperties(node, true, true))) facts |= ClassFacts.HasStaticInitializedProperties;
    ...
    function visitClassDeclaration(node) {
        ...
        if (!isClassLikeDeclarationWithTypeScriptSyntax(node) && !classOrConstructorParameterIsDecorated(legacyDecorators, node) && !isExportOfNamespace(node)) {
            return factory.updateClassDeclaration(node, visitNodes(node.modifiers, modifierVisitor, isModifier), node.name, undefined, visitNodes(node.heritageClauses, visitor, isHeritageClause), visitNodes(node.members, getClassElementVisitor(node), isClassElement));
        }
        if (promoteToIIFE) {
            context.startLexicalEnvironment();
        }
        const moveModifiers = promoteToIIFE || facts & ClassFacts.IsExportOfNamespace;
        ...
        const needsName = moveModifiers && !node.name || facts & ClassFacts.HasMemberDecorators || facts & ClassFacts.HasStaticInitializedProperties;
        const name = needsName ? node.name ?? factory.getGeneratedNameForNode(node) : node.name;
        const classDeclaration = factory.updateClassDeclaration(node, modifiers, name, undefined, visitNodes(node.heritageClauses, visitor, isHeritageClause), transformClassMembers(node));
        ...
        if (promoteToIIFE) {
            ...
            const varDecl = factory.createVariableDeclaration(factory.getLocalName(node, false, false), undefined, undefined, iife);
            setOriginalNode(varDecl, node);
            const varStatement = factory.createVariableStatement(undefined, factory.createVariableDeclarationList([
                varDecl
            ], NodeFlags.Let));
        ...
        if (moveModifiers) {
            ...
            if (facts & ClassFacts.IsNamedExternalExport) {
                return [
                    statement,
                    factory.createExternalModuleExport(factory.getDeclarationName(node, false, true))
                ];
            }
        }
        return statement;
    ...
    function visitClassExpression(node) {
        ...
        return factory.updateClassExpression(node, modifiers, node.name, undefined, visitNodes(node.heritageClauses, visitor, isHeritageClause), transformClassMembers(node));
    }
    function transformClassMembers(node) {
        ...
        if (parametersWithPropertyAssignments) {
            for (const parameter of parametersWithPropertyAssignments) {
                const parameterProperty = factory.createPropertyDeclaration(undefined, parameter.name, undefined, undefined, undefined);
    ...
    function getNewTypeMetadata(node, container) {
        if (typeSerializer) {
            let properties;
            if (shouldAddTypeMetadata(node)) {
                const typeProperty = factory.createPropertyAssignment("type", factory.createArrowFunction(undefined, undefined, [], undefined, factory.createToken(SyntaxKind.EqualsGreaterThanToken), typeSerializer.serializeTypeOfNode({
                    currentLexicalScope,
                    currentNameScope: container
                }, node, container)));
            ...
            if (shouldAddParamTypesMetadata(node)) {
                const paramTypeProperty = factory.createPropertyAssignment("paramTypes", factory.createArrowFunction(undefined, undefined, [], undefined, factory.createToken(SyntaxKind.EqualsGreaterThanToken), typeSerializer.serializeParameterTypesOfNode({
                    currentLexicalScope,
                    currentNameScope: container
                }, node, container)));
            ...
            if (shouldAddReturnTypeMetadata(node)) {
                const returnTypeProperty = factory.createPropertyAssignment("returnType", factory.createArrowFunction(undefined, undefined, [], undefined, factory.createToken(SyntaxKind.EqualsGreaterThanToken), typeSerializer.serializeReturnTypeOfNode({
                    currentLexicalScope,
                    currentNameScope: container
                }, node)));
            ...
            if (properties) {
                const typeInfoMetadata = emitHelpers().createMetadataHelper("design:typeinfo", factory.createObjectLiteralExpression(properties, true));
                return [
    ...
    function getExpressionForPropertyName(member, generateNameForComputedPropertyName) {
        ...
        if (isPrivateIdentifier(name)) {
            ...
            return generateNameForComputedPropertyName && !isSimpleInlineableExpression(name.expression) ? factory.getGeneratedNameForNode(name) : name.expression;
    ...
    function visitExpressionWithTypeArguments(node) {
        return factory.updateExpressionWithTypeArguments(node, Debug.checkDefined(visitNode(node.expression, visitor, isLeftHandSideExpression)), undefined);
    }
    ...
    function visitPropertyDeclaration(node, parent) {
        ...
        let modifiers = isClassLike(parent) ? !isAmbient ? visitNodes(node.modifiers, visitor, isModifierLike) : visitNodes(node.modifiers, modifierElidingVisitor, isModifierLike) : visitNodes(node.modifiers, decoratorElidingVisitor, isModifierLike);
        modifiers = injectClassElementTypeMetadata(modifiers, node, parent);
        if (isAmbient) {
            return factory.updatePropertyDeclaration(node, concatenate(modifiers, factory.createModifiersFromModifierFlags(ModifierFlags.Ambient)), Debug.checkDefined(visitNode(node.name, visitor, isPropertyName)), undefined, undefined, undefined);
        }
        return factory.updatePropertyDeclaration(node, modifiers, visitPropertyNameOfClassElement(node), undefined, undefined, visitNode(node.initializer, visitor, isExpression));
    }
    function visitConstructor(node) {
        if (!shouldEmitFunctionLikeDeclaration(node)) {
            return undefined;
        }
        return factory.updateConstructorDeclaration(node, undefined, visitParameterList(node.parameters, visitor, context), transformConstructorBody(node.body, node));
    }
    function transformConstructorBodyWorker(statementsOut, statementsIn, statementOffset, superPath, superPathDepth, initializerStatements) {
        ...
        if (isTryStatement(superStatement)) {
            ...
            transformConstructorBodyWorker(tryBlockStatements, superStatement.tryBlock.statements, 0, superPath, superPathDepth + 1, initializerStatements);
    ...
    function transformConstructorBody(body, constructor) {
        ...
        const prologueStatementCount = factory.copyPrologue(body.statements, statements, false, visitor);
        ...
        if (superPath.length) {
            transformConstructorBodyWorker(statements, body.statements, prologueStatementCount, superPath, 0, parameterPropertyAssignments);
        } else {
        ...
        const block = factory.createBlock(setTextRange(factory.createNodeArray(statements), body.statements), true);
    ...
    function visitMethodDeclaration(node, parent) {
        ...
        let modifiers = isClassLike(parent) ? visitNodes(node.modifiers, visitor, isModifierLike) : visitNodes(node.modifiers, decoratorElidingVisitor, isModifierLike);
        modifiers = injectClassElementTypeMetadata(modifiers, node, parent);
        return factory.updateMethodDeclaration(node, modifiers, node.asteriskToken, visitPropertyNameOfClassElement(node), undefined, undefined, visitParameterList(node.parameters, visitor, context), undefined, visitFunctionBody(node.body, visitor, context));
    }
    ...
    function visitGetAccessor(node, parent) {
        ...
        let modifiers = isClassLike(parent) ? visitNodes(node.modifiers, visitor, isModifierLike) : visitNodes(node.modifiers, decoratorElidingVisitor, isModifierLike);
        modifiers = injectClassElementTypeMetadata(modifiers, node, parent);
        return factory.updateGetAccessorDeclaration(node, modifiers, visitPropertyNameOfClassElement(node), visitParameterList(node.parameters, visitor, context), undefined, visitFunctionBody(node.body, visitor, context) || factory.createBlock([]));
    }
    ...
    function visitFunctionDeclaration(node) {
        ...
        const updated = factory.updateFunctionDeclaration(node, visitNodes(node.modifiers, modifierVisitor, isModifier), node.asteriskToken, node.name, undefined, visitParameterList(node.parameters, visitor, context), undefined, visitFunctionBody(node.body, visitor, context) || factory.createBlock([]));
    ...
    function visitFunctionExpression(node) {
        ...
        const updated = factory.updateFunctionExpression(node, visitNodes(node.modifiers, modifierVisitor, isModifier), node.asteriskToken, node.name, undefined, visitParameterList(node.parameters, visitor, context), undefined, visitFunctionBody(node.body, visitor, context) || factory.createBlock([]));
        return updated;
    }
    function visitArrowFunction(node) {
        const updated = factory.updateArrowFunction(node, visitNodes(node.modifiers, modifierVisitor, isModifier), undefined, visitParameterList(node.parameters, visitor, context), undefined, node.equalsGreaterThanToken, visitFunctionBody(node.body, visitor, context));
        return updated;
    }
    function visitParameter(node) {
        ...
        const updated = factory.updateParameterDeclaration(node, visitNodes(node.modifiers, (node) => isDecorator(node) ? visitor(node) : undefined, isModifierLike), node.dotDotDotToken, Debug.checkDefined(visitNode(node.name, visitor, isBindingName)), undefined, undefined, visitNode(node.initializer, visitor, isExpression));
        if (updated !== node) {
    ...
    function transformInitializedVariable(node) {
        ...
        if (isBindingPattern(name)) {
            ...
            return setTextRange(factory.createAssignment(getNamespaceMemberNameWithSourceMapsAndWithoutComments(name), Debug.checkDefined(visitNode(node.initializer, visitor, isExpression))), node);
        }
    }
    function visitVariableDeclaration(node) {
        const updated = factory.updateVariableDeclaration(node, Debug.checkDefined(visitNode(node.name, visitor, isBindingName)), undefined, undefined, visitNode(node.initializer, visitor, isExpression));
        if (node.type) {
    ...
    function visitCallExpression(node) {
        return factory.updateCallExpression(node, Debug.checkDefined(visitNode(node.expression, visitor, isExpression)), undefined, visitNodes(node.arguments, visitor, isExpression));
    }
    function visitNewExpression(node) {
        return factory.updateNewExpression(node, Debug.checkDefined(visitNode(node.expression, visitor, isExpression)), undefined, visitNodes(node.arguments, visitor, isExpression));
    }
    function visitTaggedTemplateExpression(node) {
        return factory.updateTaggedTemplateExpression(node, Debug.checkDefined(visitNode(node.tag, visitor, isExpression)), undefined, Debug.checkDefined(visitNode(node.template, visitor, isTemplateLiteral)));
    }
    function visitJsxSelfClosingElement(node) {
        return factory.updateJsxSelfClosingElement(node, Debug.checkDefined(visitNode(node.tagName, visitor, isJsxTagNameExpression)), undefined, Debug.checkDefined(visitNode(node.attributes, visitor, isJsxAttributes)));
    }
    function visitJsxJsxOpeningElement(node) {
        return factory.updateJsxOpeningElement(node, Debug.checkDefined(visitNode(node.tagName, visitor, isJsxTagNameExpression)), undefined, Debug.checkDefined(visitNode(node.attributes, visitor, isJsxAttributes)));
    }
    ...
    function visitEnumDeclaration(node) {
        ...
        const exportName = isExportOfNamespace(node) ? factory.getExternalModuleOrNamespaceExportName(currentNamespaceContainerName, node, false, true) : factory.getDeclarationName(node, false, true);
        ...
        const enumStatement = factory.createExpressionStatement(factory.createCallExpression(factory.createFunctionExpression(undefined, undefined, undefined, undefined, [
            factory.createParameterDeclaration(undefined, undefined, parameterName)
        ], undefined, transformEnumBody(node, containerName)), undefined, [
            moduleArg
        ]));
    ...
    function transformEnumBody(node, localName) {
        ...
        return factory.createBlock(setTextRange(factory.createNodeArray(statements), node.members), true);
    }
    function transformEnumMember(member) {
        ...
        const outerAssignment = typeof evaluated?.value === "string" || evaluated?.isSyntacticallyString ? innerAssignment : factory.createAssignment(factory.createElementAccessExpression(currentNamespaceContainerName, innerAssignment), name);
    ...
    function transformEnumMemberDeclarationValue(member, constantValue) {
        if (constantValue !== undefined) {
            return typeof constantValue === "string" ? factory.createStringLiteral(constantValue) : constantValue < 0 ? factory.createPrefixUnaryExpression(SyntaxKind.MinusToken, factory.createNumericLiteral(-constantValue)) : factory.createNumericLiteral(constantValue);
        } else {
    ...
    function addVarForEnumOrModuleDeclaration(statements, node) {
        const varDecl = factory.createVariableDeclaration(factory.getLocalName(node, false, true));
        const varFlags = currentLexicalScope.kind === SyntaxKind.SourceFile ? NodeFlags.None : NodeFlags.Let;
        const statement = factory.createVariableStatement(visitNodes(node.modifiers, modifierVisitor, isModifier), factory.createVariableDeclarationList([
            varDecl
        ], varFlags));
        setOriginalNode(varDecl, node);
    ...
    function visitModuleDeclaration(node) {
        ...
        const moduleStatement = factory.createExpressionStatement(factory.createCallExpression(factory.createFunctionExpression(undefined, undefined, undefined, undefined, [
            factory.createParameterDeclaration(undefined, undefined, parameterName)
        ], undefined, transformModuleBody(node, containerName)), undefined, [
            moduleArg
        ]));
    ...
    function transformModuleBody(node, namespaceLocalName) {
        ...
        currentScopeFirstDeclarationsOfName = savedCurrentScopeFirstDeclarationsOfName;
        const block = factory.createBlock(setTextRange(factory.createNodeArray(statements), statementsLocation), true);
    ...
    function visitImportDeclaration(node) {
        ...
        return importClause ? factory.updateImportDeclaration(node, undefined, importClause, node.moduleSpecifier, node.attributes) : undefined;
    }
    ...
    function visitExportAssignment(node) {
        return compilerOptions.verbatimModuleSyntax || resolver.isValueAliasDeclaration(node) ? visitEachChild(node, visitor, context) : undefined;
    }
    function visitExportDeclaration(node) {
        ...
        return exportClause ? factory.updateExportDeclaration(node, undefined, node.isTypeOnly, exportClause, node.moduleSpecifier, node.attributes) : undefined;
    }
    ...
    function visitImportEqualsDeclaration(node) {
        ...
        if (isNamedExternalModuleExport(node) || !isExportOfNamespace(node)) {
            return setOriginalNode(setTextRange(factory.createVariableStatement(visitNodes(node.modifiers, modifierVisitor, isModifier), factory.createVariableDeclarationList([
                setOriginalNode(factory.createVariableDeclaration(node.name, undefined, undefined, moduleReference), node)
            ])), node), node);
        } else {
    ...
    function createExportMemberAssignmentStatement(node) {
        const expression = factory.createAssignment(factory.getExternalModuleOrNamespaceExportName(currentNamespaceContainerName, node, false, true), factory.getLocalName(node));
    ...
    function createNamespaceExport(exportName, exportValue, location) {
        return setTextRange(factory.createExpressionStatement(factory.createAssignment(factory.getNamespaceMemberName(currentNamespaceContainerName, exportName, false, true), exportValue)), location);
    }
    ...
    function trySubstituteNamespaceExportedName(node) {
        if (enabledSubstitutions & applicableSubstitutions && !isGeneratedIdentifier(node) && !isLocalName(node)) {
            ...
            if (container && container.kind !== SyntaxKind.SourceFile) {
                const substitute = applicableSubstitutions & TypeScriptSubstitutionFlags.NamespaceExports && container.kind === SyntaxKind.ModuleDeclaration || applicableSubstitutions & TypeScriptSubstitutionFlags.NonQualifiedEnumMembers && container.kind === SyntaxKind.EnumDeclaration;
                if (substitute) {
                    return setTextRange(factory.createPropertyAccessExpression(factory.getGeneratedNameForNode(container), node), node);
                }
            }
        }
        return undefined;
    ...
    function substituteConstantValue(node) {
        ...
        if (constantValue !== undefined) {
            ...
            const substitute = typeof constantValue === "string" ? factory.createStringLiteral(constantValue) : constantValue < 0 ? factory.createPrefixUnaryExpression(SyntaxKind.MinusToken, factory.createNumericLiteral(-constantValue)) : factory.createNumericLiteral(constantValue);
